...
class Query:
    ...
    def refine(self):
        ...
        if self.hits > self.hit_cap:
            return self.__refine_lower()
        return self.__refine_upper()

    def has_upper(self):
        if self.price_to is None and self.hits <= self.hit_cap: return False
        if self.price_to == self.max_price: return False
        return True

    def __refine_lower(self):
        if self.price_to is None:
            if self.price_from is not None: self.query['priceto'] = [str(self.price_from * 2)]
            else: self.query['priceto'] = ['100000']
        else:
    ...
    def __get_divider(self, price_range: int):
        if price_range > 500000:  return 8
        if price_range > 100000: return 4
        return 2
